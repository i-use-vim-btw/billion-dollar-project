...
coll_name = "test121"
host = input("enter milvus location: ")
base_url = input("Enter router url: ")
sdk = Gandi(access_token="YOUR_ACCESS_TOKEN", base_url=base_url)
...
result = sdk.vectors.delete(
    VectorsDeleteRequest(collection_name=coll_name,
                         filter="id in [2, 4]", host=host)
)
...
print(sdk.collections.has(CollectionHasRequest(
    collection_name=coll_name, host=host)))
...
print(
    sdk.collections.drop(CollectionDropRequest(
        collection_name="hello123", host=host))
)
